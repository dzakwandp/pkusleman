<template>
  <div class="flex flex-col px-4 md:px-20 mt-10 min-h-screen text-gray-800 pb-20">
    ...
    <div v-else-if="!article" class="flex flex-col items-center py-20">
      ...
      <router-link to="/artikel-berita" class="text-primary-green hover:underline">
    ...
    <div v-else class="flex flex-col w-full max-w-5xl mx-auto">
      <!-- Back Button -->
      <router-link to="/artikel-berita" class="inline-flex items-center text-sm text-gray-500 hover:text-primary-green mb-6 w-fit transition-colors">
        <span class="mr-1">←</span> Kembali ke Artikel & Berita
      </router-link>

      <!-- Article Header -->
      <div class="flex flex-col gap-4 mb-8">
        <h1 class="text-3xl md:text-5xl font-bold leading-tight">{{ article.judul }}</h1>
        <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500">
          <span>{{ moment(article.createdAt).format("DD MMMM YYYY") }}</span>
          <span v-if="parsedTags.length > 0" class="flex gap-2">
            <span v-for="tag in parsedTags" :key="tag" class="bg-gray-100 px-3 py-1 rounded-full text-xs font-medium">
              {{ tag }}
            </span>
          </span>
        </div>
      ...
      <!-- Hero Image -->
      <div class="w-full h-[300px] md:h-[500px] rounded-2xl overflow-hidden mb-10 shadow-lg">
        <img :src="article.gambar" class="w-full h-full object-cover" :alt="article.judul" />
      </div>

      <!-- Article Content -->
      <!-- Added prose classes for better formatting if tailwind typography plugin is available, otherwise basic styling -->
      <div class="prose max-w-none text-lg leading-relaxed text-gray-700 article-content" v-html="article.konten"></div>
      ...
      <div class="flex flex-col gap-6">
        <h2 class="text-2xl md:text-3xl font-bold border-l-4 border-primary-green pl-4">Artikel Terbaru</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          <router-link
            ...
            <div class="flex flex-col flex-grow relative z-20">
              <h3 class="text-lg font-bold line-clamp-2 leading-tight mb-2" :title="item.judul">{{ item.judul }}</h3>
              
              <div class="flex justify-between items-center text-xs text-gray-500 mb-3 mt-auto">
                <span class="font-medium line-clamp-1 max-w-[60%]">{{ formatTagsDisplay(item.tags) }}</span>
                <span class="shrink-0">{{ moment(item.createdAt).format("DD MMM YYYY") }}</span>
              </div>
              
              <p class="text-sm text-gray-600 line-clamp-3 mb-4" v-html="item.konten ? item.konten.substring(0, 120) + '...' : ''"></p>
              
              <div class="mt-auto pointer-events-none">
                <Button class="text-xs rounded-lg w-full" text="Baca Selengkapnya" />
              </div>
            </div>
          </router-link>
        </div>

        <div v-if="latestArticles.length === 0" class="text-gray-500 italic mt-2">
    ...
    article: {
      ...
      handler(newArticle) {
        if (newArticle) {
          ...
          const plainTextDesc = newArticle.konten ? newArticle.konten.replace(/<[^>]*>?/gm, '').substring(0, 160) + '...' : 'Baca artikel lengkap di RS PKU Muhammadiyah Sleman.';
          
          useHead({
            title: newArticle.judul,
            meta: [
