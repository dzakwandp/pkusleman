<template>
  <div class="flex flex-col p-4 md:px-20 mt-10 min-h-screen">
    <div class="flex flex-col w-max mx-auto gap-2">
      <p class="text-4xl text-center text-gray-800">Layanan IGD</p>
    ...
    <div v-if="loaded">
      ...
      <div class="flex flex-col rounded-lg shadow-lg my-10">
        <p class="bg-primary-green rounded-t-lg text-white font-semibold p-2">
          Fasilitas {{ data[0].nama_poli }}
        </p>
        <div class="flex flex-col gap-2 p-2 list-class" v-html="data[0].daftar_layanan" />
      </div>
    </div>
  </div>
</template>
    ...
    async getData() {
      try {
        const data = await axios.get(
          "https://apiweb.pkusleman.com/api/layanan/igd"
        );
        this.data = data.data;
